...
describe('createUser()', () => {
  ...
  it('TC_AUTH_01 - should_create_user_successfully_with_valid_data', async () => {
    ...
    expect(pool.query).toHaveBeenNthCalledWith(
      ...
      expect.arrayContaining(['Test User', 'test@example.com', hashedPw, '0901234567'])
    );
...
describe('updatePassword()', () => {
  ...
  it('TC_AUTH_09 - should_hash_new_password_and_delete_token_on_success', async () => {
    const resetRecord = { user_id: 3, token: 'validtoken123', email: 'u@test.com' };
...
describe('verifyResetToken()', () => {
  ...
  it('TC_AUTH_10 - should_throw_INVALID_TOKEN_when_token_expired_or_not_found', async () => {
    ...
    await expect(verifyResetToken('expiredOrInvalidToken')).rejects.toMatchObject({
