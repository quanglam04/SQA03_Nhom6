...
async function getEmbeddingCached(text) {
  ...
  try {
    ...
    console.log(`[CACHE] Saved, size: ${embeddingCache.size}/${MAX_CACHE_SIZE}`);
    return embedding;
  } catch (err) {
...
async function getRAGProductsFromDB(userQuestion, topKProducts = 5) {
  try {
    ...
    for (let i = 0; i < Math.min(3, metadatas.length); i++) {
      console.log(`  [${i}]: type=${metadatas[i]?.type}, id=${metadatas[i]?.id}, variant=${metadatas[i]?.variant_name}, price=${metadatas[i]?.price}`);
    }
    ...
    console.log(`[DEBUG] Extracted ${products.length} variants directly from Chroma metadata`);
...
function detectProductQuery(userMessage) {
  ...
  const productKeywords = [
    'sản phẩm', 'mua', 'giá', 'tồn kho', 'stock', 'danh sách',
    'có', 'gì', 'nào', 'như thế nào', 'loại nào', 'cái nào',
    'từ vịt', 'từ gà', 'từ heo', 'từ cá', 'hải sản',
    'chả', 'nem', 'ruốc', 'hạt', 'bánh',
    'tìm', 'kiếm', 'gợi ý', 'recommend', 'đề xuất'
  ];
  
  // Greeting/small talk (NOT product query)
  const greetingKeywords = [
    'xin chào', 'hello', 'hi', 'chào', 'cảm ơn', 'thank', 'tạm biệt',
    'bye', 'goodbye', 'ơi', 'được không', 'được không', 'sao'
  ];
...
function buildPromptStructuredNatural(userQuestion, products) {
  // Improved: Include variant_name for each product
  const productList = products.map((p, idx) => {
    const variantInfo = p.variant_name ? ` (${p.variant_name})` : '';
    return `${idx + 1}. ${p.name}${variantInfo} - Danh mục: ${p.category} - Giá: ${p.price.toLocaleString('vi-VN')} VNĐ - Tồn kho: ${p.stock} ${p.unit}`;
  }).join('\n');
...
- Nếu khách hỏi về sản phẩm không có trong cửa hàng, hãy nói rằng sản phẩm đó hiện không có sẵn và gợi ý các sản phẩm thay thế từ danh sách
...
async function callGeminiRest(recentMessages, finalPrompt, temperature = 0.5, max_tokens = 800, retryCount = 0) {
  ...
  try {
    ...
    if (err.status === 503 && retryCount < 2) {
      console.log(`WARN: API overloaded (503). Retrying... (attempt ${retryCount + 2}/3)`);
      await new Promise(resolve => setTimeout(resolve, 2000)); // Wait 2 seconds
      return callGeminiRest(recentMessages, finalPrompt, temperature, max_tokens, retryCount + 1);
    }
...
async function chatHandler(req, res) {
  ...
  try {
    ...
    if (!messages && !prompt) return res.status(400).json({ success: false, message: 'messages or prompt required' });
    ...
    if (isProductQuery) {
      ...
      console.log(`Time (RAG): ${ragMs}ms, Context length: ${ragContext.length} chars`);
    ...
    console.log(`Calling Gemini (temperature: ${temperature}, messages: ${recentMessages.length})`);
    
    reply = await callGeminiRest(recentMessages, finalPrompt, temperature, max_tokens);
    
    const geminiEndTime = Date.now();
    const geminiMs = geminiEndTime - geminiStartTime;
    console.log(`Time (Gemini API): ${geminiMs}ms, Reply length: ${reply.length} chars`);
    ...
    console.log(`Breakdown: Intent=${intentMs}ms, RAG=${ragMs}ms, DB=${dbMs}ms, Gemini=${geminiMs}ms`);
